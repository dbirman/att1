...
if isfield (settings, 'exp_setup') && strcmp(settings.exp_setup, 'dj office')
    ...
    % Where figures are plotted to (you dont want git repo path for
    % figures):
    settings.path_baseline_figures = sprintf('~/Box Sync/HUMMON_DATA/');
    
    % "Experiments_data" folder with eyelink and psychtoolbox data:
    settings.path_baseline_data = sprintf('~/Box Sync/HUMMON_DATA/');
...
% Generate path names (no changes here needed)
