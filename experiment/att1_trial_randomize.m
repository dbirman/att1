...
elseif strcmp(expsetup.stim.esetup_exp_version{tid}, 'release lever on big ring')
    ...
    if strcmp(expsetup.stim.training_stage_matrix, 'single target same orientation one ring')
        temp1(temp1==2)=NaN; % Only one ring is shown, random order
    end
    if strcmp(expsetup.stim.training_stage_matrix, 'single target orientation change one ring')
